...
11. **Setup mode?**
    ...
    - **Full** — all 13 skills/ files, pre-commit hooks, graphify, agentation, AGENTS.md. Best for: team projects, production-bound work.
...
**Rule:** Every exported function, type, and constant must have a doc comment. `go vet` is non-negotiable in pre-commit.
...
#### Go
```go
// Distinct named types instead of bare string — the compiler enforces separation.
type UserID string
type OrgID  string
type Email  string

// Constructor validates and is the only blessed path to a value.
func NewUserID(raw string) (UserID, error) {
    if !userIDPattern.MatchString(raw) {
        return "", fmt.Errorf("invalid UserID: %q", raw)
    }
    return UserID(raw), nil
}

// func GetUser(id UserID) ...  — passing an OrgID is a compile error.
```

#### Rust
```rust
// The newtype pattern — zero-cost at runtime, fully distinct at compile time.
...
impl UserId {
    pub fn parse(raw: &str) -> Result<Self, ParseError> {
        if !is_valid_user_id(raw) {
            return Err(ParseError::UserId(raw.to_owned()));
        }
        Ok(Self(raw.to_owned()))
    }
}
// fn get_user(id: UserId) — handing it an OrgId will not compile.
```
...
1. **Safety** — no secrets staged, no `plan/` staged, no `any` (TS), no `console.log` (TS), no files over 500 lines
2. **Python** (if in stack) — ruff format → ruff lint → mypy
3. **TypeScript** (if in stack) — prettier → eslint → tsc --noEmit
4. **Go** (if in stack) — gofmt → golangci-lint → go vet
5. **Rust** (if in stack) — cargo fmt → cargo clippy → cargo check
6. **Tests** — fast unit tests only. Integration tests belong in CI only.
7. **Summary** — colored pass/fail count, exit 1 if any failure
...
- One language-specific file (`skills/typescript.md` OR `skills/python.md` OR `skills/go.md` OR `skills/rust.md`)
...
4. **Non-negotiables** — pulled from Part II, adapted to the confirmed stack:
   - Always `===` not `==` (TS) / always `==` not `is` for values (Python)
   - No `any`, no non-null assertions (TS) / `mypy strict` (Python)
...
**Full mode only:**
```
□ All 13 skills/ files created (or marked N/A for stack)?
...
> II.11 → skills/typescript.md + skills/python.md (equality, type safety)
...
**Rule:** Use `hmac.compare_digest` / `crypto.timingSafeEqual` for: HMAC digests, API keys, password reset tokens, session tokens. Regular `==` for everything else.
...
**Fix:** Use RE2-based engines (`google-re2` Python package, Rust `regex` crate) — O(n) time, no backtracking. Lint regexes in CI with `safe-regex`. If users supply regex patterns, run them in a thread with a timeout.
...
Coverage measures which lines were executed, not whether the behavior is correct. The real metric is **mutation testing** — `mutmut` (Python), `Stryker` (JS/TS). Mutants flip operators (`>` to `>=`, `+` to `-`). If your tests don't kill the mutant, you have a blind spot.
...
**Tooling:** `radon cc -a .` for Python, `eslint complexity` rule for TypeScript.
